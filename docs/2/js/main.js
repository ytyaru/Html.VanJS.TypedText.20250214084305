...
//        p('A text format that defines the name, order, type, and initial value of the value.'),
    ...
    */
    ...
    // 単一行（値あり）
    a.e(TypeError, `k（(）は次のパターンのみ有効です。:/^[a-zA-Z][a-zA-Z0-9]*$/`, ()=>TypedText.parse('('));
    a.e(TypeError, `k（()）は次のパターンのみ有効です。:/^[a-zA-Z][a-zA-Z0-9]*$/`, ()=>TypedText.parse('()'));
    a.e(TypeError, `k（(a)）は次のパターンのみ有効です。:/^[a-zA-Z][a-zA-Z0-9]*$/`, ()=>TypedText.parse('(a)'));
    a.e(TypeError, `k（(a)）は次のパターンのみ有効です。:/^[a-zA-Z][a-zA-Z0-9]*$/`, ()=>TypedText.parse('(a) '));
    a.t(()=>{
        const tt = TypedText.parse('(name) Yamada');
        console.log(tt)
        return 1===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'name'===tt.ktds[0].k && null===tt.ktds[0].t && null===tt.ktds[0].d
            && 1===tt.vals.length && 'Yamada'===tt.vals[0]
    })
    a.t(()=>{
        const tt = TypedText.parse('(name:str) Yamada');
        return 1===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'name'===tt.ktds[0].k && 'str'===tt.ktds[0].t && null===tt.ktds[0].d
            && 1===tt.vals.length && 'Yamada'===tt.vals[0]
    })
    a.t(()=>{
        const tt = TypedText.parse('(name=A) Yamada');
        return 1===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'name'===tt.ktds[0].k && null===tt.ktds[0].t && 'A'===tt.ktds[0].d
            && 1===tt.vals.length && 'Yamada'===tt.vals[0]
    })
    a.t(()=>{
        const tt = TypedText.parse('(name:str=A) Yamada');
        return 1===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'name'===tt.ktds[0].k && 'str'===tt.ktds[0].t && 'A'===tt.ktds[0].d
            && 1===tt.vals.length && 'Yamada'===tt.vals[0]
    })
    a.t(()=>{
        const tt = TypedText.parse('(name age) Yamada 12');
        console.log(tt)
        return 2===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'name'===tt.ktds[0].k && null===tt.ktds[0].t && null===tt.ktds[0].d
            && 'age'===tt.ktds[1].k && null===tt.ktds[1].t && null===tt.ktds[1].d
            && 2===tt.vals.length && 'Yamada'===tt.vals[0] && '12'===tt.vals[1]
    })
    a.t(()=>{
        const tt = TypedText.parse('(name age:int) Yamada 12');
        return 2===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'name'===tt.ktds[0].k && null===tt.ktds[0].t && null===tt.ktds[0].d
            && 'age'===tt.ktds[1].k && 'int'===tt.ktds[1].t && null===tt.ktds[1].d
            && 2===tt.vals.length && 'Yamada'===tt.vals[0] && '12'===tt.vals[1]
    ...
    a.t(()=>{
        const tt = TypedText.parse('(name age=12) Yamada 13');
        return 2===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'name'===tt.ktds[0].k && null===tt.ktds[0].t && null===tt.ktds[0].d
            && 'age'===tt.ktds[1].k && null===tt.ktds[1].t && '12'===tt.ktds[1].d
            && 2===tt.vals.length && 'Yamada'===tt.vals[0] && '13'===tt.vals[1]
    })
    a.t(()=>{
        const tt = TypedText.parse('(name age:int=12) Yamada 13');
        return 2===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'name'===tt.ktds[0].k && null===tt.ktds[0].t && null===tt.ktds[0].d
            && 'age'===tt.ktds[1].k && 'int'===tt.ktds[1].t && '12'===tt.ktds[1].d
            && 2===tt.vals.length && 'Yamada'===tt.vals[0] && '13'===tt.vals[1]
    ...
    a.t(()=>{
        const tt = TypedText.parse('(id:I=0 name:s=Yamada age:i=12 isMale:b=v weight:f=65.1) 1 Yamada 13 _ 72.3');
        console.log(tt)
        return 5===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'id'===tt.ktds[0].k && 'I'===tt.ktds[0].t && '0'===tt.ktds[0].d
            && 'name'===tt.ktds[1].k && 's'===tt.ktds[1].t && 'Yamada'===tt.ktds[1].d
            ...
            && 'isMale'===tt.ktds[3].k && 'b'===tt.ktds[3].t && 'v'===tt.ktds[3].d
            && 'weight'===tt.ktds[4].k && 'f'===tt.ktds[4].t && '65.1'===tt.ktds[4].d
            && 5===tt.vals.length && '1'===tt.vals[0] && 'Yamada'===tt.vals[1] && '13'===tt.vals[2] && '_'===tt.vals[3] && '72.3'===tt.vals[4]
    })
    a.t(()=>{
        const tt = TypedText.parse('(id:I=0 name:s=Yamada age:i=12 isMale:b=v weight:f=65.1)     ');
        console.log(tt)
        return 5===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            ...
            && 'isMale'===tt.ktds[3].k && 'b'===tt.ktds[3].t && 'v'===tt.ktds[3].d
            && 'weight'===tt.ktds[4].k && 'f'===tt.ktds[4].t && '65.1'===tt.ktds[4].d
            && 5===tt.vals.length && ''===tt.vals[0] && ''===tt.vals[1] && ''===tt.vals[2] && ''===tt.vals[3] && ''===tt.vals[4]
    })
    a.t(()=>{
        const tt = TypedText.parse('(id:I=0 name:s=Yamada age:i=12 isMale:b=v weight:f=65.1)   13  ');
        console.log(tt)
        return 5===tt.ktds.length && TypedText.Formats.OneLine===tt.format
            && 'id'===tt.ktds[0].k && 'I'===tt.ktds[0].t && '0'===tt.ktds[0].d
            && 'name'===tt.ktds[1].k && 's'===tt.ktds[1].t && 'Yamada'===tt.ktds[1].d
            ...
            && 'isMale'===tt.ktds[3].k && 'b'===tt.ktds[3].t && 'v'===tt.ktds[3].d
            && 'weight'===tt.ktds[4].k && 'f'===tt.ktds[4].t && '65.1'===tt.ktds[4].d
            && 5===tt.vals.length && ''===tt.vals[0] && ''===tt.vals[1] && '13'===tt.vals[2] && ''===tt.vals[3] && ''===tt.vals[4]
    })
    // 複数行
    a.t(()=>{
...
2	Suzuki	24	_	72.3`);
    ...
    a.t(()=>{
        const tt = TypedText.parse('name');
        tt.format = TypedText.Formats.MultiLine
        console.log(tt.vals)
        return 1===tt.ktds.length && TypedText.Formats.OneLine===tt.format && 'name'===tt.ktds[0].k && null===tt.ktds[0].t && null===tt.ktds[0].d && 0===tt.vals.length
    })
