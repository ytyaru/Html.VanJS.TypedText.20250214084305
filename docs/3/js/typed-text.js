...
class TypedText {
    ...
    static #parseOneLine(text, isMutable=false) {
        const match = text.match(/^\((?<typeText>[^\(\)]+)\) (?<valueText>.+)$/);
        if (match) {// (name:str=Nanashi age:int=-1) Yamada 12
            const typeTexts = Splitter.split(match.groups.typeText);
            const ktds = typeTexts.map(v=>Ktd.parse(v, isMutable));
            return new TypedText(
                ktds,
                Splitter.split(match.groups.valueText),
                this.Formats.OneLine, this.#NOT_CALL_CONSTRUCTOR)
        } else {
            const ktds = Splitter.split(text).map(v=>Ktd.parse(v, isMutable));
            return new TypedText(ktds, [], this.Formats.OneLine, this.#NOT_CALL_CONSTRUCTOR)
        }
    }
...
class Ktd {// KeyTypeDefault key:type=valueを三値に分離する（keyは必須。type,valueは任意）
    ...
    static format(k, t, d) {Ktd.#throwKey(k);return `${k}${t ? ':'+t : ''}${d ? '='+d : ''}`;}
